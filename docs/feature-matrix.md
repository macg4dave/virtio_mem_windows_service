...
| RHEL-controlled cross-platform developer gate | Complete | Both | Ops | Bash + VS Code tasks | Latest gates pass 47 core, 58 host, and 67 native Windows tests with release builds, formatting, and warnings-as-errors Clippy |
...
| Quantitative desired-allocation model | In Progress | Host policy | Contract | Rust | Normative M10e contract defines checked physical/commit candidates, fixed-visible-base validation, effective maximum, reserve defaults, 10-minute high-water history, 256 MiB hysteresis, safe floors, and restart warm-up |
...
| Single-VM recovery matrix | In Progress | Host | Integration | Rust + Bash | Hermetic cancellation, restart, interruption, ownership-conflict, typed preflight/unknown-command, stall-latch, and no-replay paths pass; installed candidate rejection/no-replay and one-shot recovery are live-qualified, while attestation regeneration and active-controller reboot/interruption remain |
