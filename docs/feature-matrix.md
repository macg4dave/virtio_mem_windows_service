...
| Pressure-aware demand target | Shadow host integration implemented; qualification pending | Versioned requirement, notification-led pressure, shrink safety, history, separate policy fingerprint, append-only comparison evidence, and status exposure are wired to accepted telemetry and alias-scoped live state. Shadow mode always returns `NoChange`; local and native Windows gates pass, while deployment workload correlation and review remain. |
...
| Versioned controller status | Version 2 implemented; refreshed live read pending | The read-only host command joins fresh alias-scoped live state with the matching durable checkpoint and now exposes pressure shadow state alongside telemetry, reclaim, capacity, ownership, latch, and recovery without mutation. The earlier version-1 live check preserved the inactive, disabled service state. |
