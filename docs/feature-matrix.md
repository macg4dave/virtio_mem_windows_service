...
| Validate virtio-mem state | In Progress | Host | Ops | Bash + Rust | Canonical byte-based VirtioMemState validation, checked KiB/XML boundaries, device/block alignment, maximum-value tests, live XML discovery, alias selection, and requested/current convergence are implemented; live compatibility/workload and failure-mode validation remain |
...
| RHEL-controlled cross-platform developer gate | In Progress | Both | Ops | Bash + VS Code tasks | Windows OpenSSH/MSVC endpoint and fingerprint-pinned evidence runner are ready; completion requires two recorded RHEL-controlled aggregate runs with checksum-verified artifacts |
