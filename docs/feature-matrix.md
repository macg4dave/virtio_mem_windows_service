...
| RHEL-controlled cross-platform developer gate | Complete | Both | Ops | Bash + VS Code tasks | Latest local RHEL gate passes 31 core and 40 host tests; the prior native Windows gate passes 64 tests, while TASK-020's Windows changes await a refreshed native gate |
...
| Current-allocation/report join | In Progress | Both | Contract | Rust | M10c implementation publishes raw Windows telemetry and joins its validated VM/time envelope with alias-scoped live libvirt `current` on the host; local core/host gates pass and native Windows validation remains |
