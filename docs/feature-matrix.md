...
| RHEL-controlled cross-platform developer gate | Complete | Both | Ops | Bash + VS Code tasks | Latest gates pass 46 core, 50 host, and 67 native Windows tests with release builds, formatting, and warnings-as-errors Clippy |
...
| Quantitative desired-allocation model | Planned | Host policy | Contract | Rust | M10e will derive one absolute target from fresh Windows availability/commit demand, explicit fixed/base memory, reserves, rolling history, hysteresis, safe floors, limits, and alignment |
| Desired/requested/current reconciliation | Planned | Host | Controller | Rust | M10f will preserve current as accounting authority, safely raise or cancel a pending shrink on renewed pressure, prohibit another lower in-flight target, and expose constrained partial/no-progress reclaim |
...
| Windows automatic shrink | Implemented; disabled after live qualification | Host + driver | Integration | Rust + Bash | One-block retry/recovery passed, but an automatic 64 MiB request made no progress for 300 seconds; a paced 256 MiB ramp had earlier reclaimed about 2.93 GiB before stalling; growth remains enabled and automatic reclaim stays off |
| Single-VM recovery matrix | In Progress | Host | Integration | Rust + Bash | One-shot abandon-to-current now requires two stable samples, an immediate reread, one request, and 30-second convergence; live wakeup/recovery and wider interruption matrix remain |
