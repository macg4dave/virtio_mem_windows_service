...
`tools/xtask` is the repository's authoritative build, test, remote-Windows,
and bounded live-validation control plane. Run it from the repository root
through the Cargo alias:
...
tool on RHEL. `cargo xtask gate all` adds the explicitly configured native
Windows gate. It does not treat a Linux build as Windows evidence.
